...
#1) Change sha1_hash to SHA-1 in the downloaded csvquote
#2) sqlite3 malware.db
#3) sqlite>  .mode csv
#4) sqlite> .import review.csv review
#5) sqlite> CREATE INDEX `sha1` ON `review` ( `SHA-1` COLLATE NOCASE );
#6) sqlite> CREATE INDEX `filename`  ON `review` ( `FileName` COLLATE NOCASE );
#7) sqlite> .exit
       ...
       try: 
            with reviewcon:
                ...
                if row:
                    ...
                    dataline.append(row["FileName"])
                    dataline.append("")
                    dataline.append("VT %: " +  row["vtpercent"] + "  https://virustotal.com/gui/file/"+row["sha256_hash"])
                    dataline.append("")  
                    dataline.append("")
