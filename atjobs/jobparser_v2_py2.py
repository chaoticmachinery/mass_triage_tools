...
class Job:
    ...
    def __repr__(self):
        ...
        #if self.Test != '':
        if hasattr(self, "JobSignature") and self.Test != '':
            str = ""
            for ch in self.JobSignature:
                str += hex(ord(ch)).lstrip("0x")
...
def main():
    ...
    # Allow user to define output files via new options: -o (output) and -s (summary)
    try:
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            ...
            print("  -s, --summary  Specify summary text file (pipe-delimited UUID|App|Params)")
            sys.exit(2)
        elif o in ("-f", "--file"):
            if os.path.isfile(a):
                file = open(a, "rb")
    ...
    #with open(output_file, "w", encoding="utf-8") as out, open(summary_file, "w", encoding="utf-8") as summary:
    with codecs.open(output_file, "a", "utf-8") as out, codecs.open(summary_file, "a", "utf-8") as summary:
        ...
        def process_job(job, filename):
            ...
            # Write to summary (pipe-delimited)
            uuid = str(job.UUID)
        ...
        if dir and os.path.isdir(dir):
            ...
            job = Job(data)
            process_job(job, file.name)

    #print("\n[+] All output written to {}".format(output_file))
