...
class Job:
    ...
    def __repr__(self):
        lines = ""
        lines += "Product Info: {0}|".format(products.get(self.ProductInfo, "None"))
        lines += "File Version: {0}|".format(self.FileVersion)
        lines += "UUID: {0}|".format(self.UUID)
        priority = ""
        for p in priorities:
            if self.Priority & p == p:
                priority += priorities[p] + ", "
        if priority != "":
            lines += "Priorities: {0}|".format(priority.rstrip(", "))
        hours, ms = divmod(self.MaxRunTime, 3600000)
        minutes, ms = divmod(ms, 60000)
        seconds = ms / 1000
        lines += "Maximum Run Time: {0:02}:{1:02}:{2:02}.{3} (HH:MM:SS.MS)|".format(hours, minutes, seconds, ms)
        lines += "Exit Code: {0}|".format(self.ExitCode)
        lines += "Status: {0}|".format(task_status.get(self.Status, "Unknown Status"))
        theflags = ""
        for flag in flags:
            if self.Flags & flag == flag:
                theflags += flags[flag] + ", "
        lines += "Flags: {0}|".format(theflags.rstrip(", "))
        lines += "Date Run: {0}|".format(self.RunDate)
        lines += "Running Instances: {0}|".format(self.RunningInstanceCount)
        lines += "Application: {0}|".format(self.Name)
        if self.Parameter != "":
            lines += "Parameters: {0}|".format(self.Parameter)
        lines += "Working Directory: {0}|".format(self.WorkingDirectory)
        lines += "User: {0}|".format(self.User)
        lines += "Comment: {0}|".format(self.Comment)
        lines += "Scheduled Date: {0}".format(self.ScheduledDate)
        return lines
