...
#====================================================
# Old NSRL Text Style DB generation
#====================================================
#Create Old Text Style NSRL DB
...
#3) ./csv_to_sqlite.py -f nsrl.csv -f NSRLMfg.csv -f NSRLOS.csv -f NSRLProd.csv -o ./nsrl.db
#====================================================

#====================================================
#NSRL SQLite Version
#
# This applies to all versions of the NSRL sqlite databases
#====================================================
#1) sqlite3 {db}
#2) sqlite> CREATE INDEX `sha1` ON `FILE` ( `sha1` COLLATE NOCASE );
#3) sqlite> CREATE INDEX `code` ON `prod` ( `ProductCode`  COLLATE NOCASE );
#4) sqlite> CREATE INDEX `apptype` ON `prod` ( `ApplicationType`  COLLATE NOCASE );
#5) sqlite> CREATE INDEX `OpSystemCode` ON `prod` ( `OpSystemCode`  COLLATE NOCASE );
#6) sqlite> .exit
#====================================================

#====================================================
#Malware Bazaar DB Creation
#====================================================
#Old Note: 1) Change sha1_hash to SHA-1 in the downloaded csvquote; remove commented lines in the beginning of file and uncomment the fieldnames
#0) wget https://bazaar.abuse.ch/export/csv/full/
#1) remove commented lines in the beginning of file and uncomment the fieldnames
#2) sed 's/, /,/g' full.csv > full3.csv
#2) sqlite3 malware.db
#3) sqlite> .mode csv
#4) sqlite> .import full3.csv malware
#5) sqlite> CREATE INDEX `sha1` ON `malware` ( `sha1_hash` COLLATE NOCASE );
#6) sqlite> CREATE INDEX `filename`  ON `malware` ( `FileName` COLLATE NOCASE );
#7) sqlite> .exit
#====================================================
...
#2old) grep AmCache  amcache_2019-04-24.txt | awk  -v OFS="\|" -F"\|" '{print $8, $7}' >> review.csv 
#2) grep -v nmap amcache_2020-03-01.txt | fgrep -f amkeys | awk  -v OFS="\|" -F"\|" '{print $9, $8}' >> review.csv
#3) sqlite3 review.db
#4) sqlite>  .mode csv
#5) sqlite>  .separator "\|"
...
# Creating Indexes for new version of NSRL using sqlite3
#1) CREATE INDEX `sha1` ON `FILE` ( `sha1` COLLATE NOCASE );
#2) CREATE INDEX `filename` ON `FILE` ( `file_name` COLLATE NOCASE );
...
#White/Review List combine DBs
# sqlite3 current.db
#SQLite version 3.26.0 2018-12-01 12:34:55
#Enter ".help" for usage hints.
#sqlite> attach './old.db' as toMerge;
#sqlite> insert into review  select * from toMerge.review;
...
def nsrllookup(lineimport,writedata,reviewlist):
 ...
 #note  if no rows then nsrlend otherwise continue.             
              sqlstr = 'SELECT * from FILE WHERE "sha1" = ?' + combinesettings
              #print("SQL: "+str(sqlstr)+"   "+str(srchline)+"\n") 
              cur.execute(sqlstr, (srchline,) )
              row = cur.fetchone()
              if row is not None:
                 #dataline.append(linedata[0].decode("utf8"))
                 #dataline.append("NSRLEND") # Src DB: "+str(dbitem))
              #else:
                 #if match > 0:
                 #   dataline.append("\n")
                 match += 1
                 dataline.append(linedata[0].decode("utf8"))
                 dataline.append(row["sha256"])
                 dataline.append(row["md5"])
                 dataline.append(row["crc32"])
                 dataline.append(row["file_name"]) #Filename from DB
                 dataline.append(row["file_size"])
                 try:
                    sqlstr2 = 'SELECT * from PKG WHERE package_id = ? '  + combinesettings
                    cur.execute(sqlstr2, (row["package_id"],) )
                    rowsprod = cur.fetchone()

                    if rowsprod:
                      dataline.append(rowsprod["application_type"])
                      dataline.append(rowsprod["name"])
                      dataline.append(rowsprod["version"])  
               
                 except Exception as e:
                    print("Database Prod table in NSRL db - Error: %s" % e)
                    sys.exit(1)
                 dataline.append("NSRL Src DB: "+str(dbitem))
                 writedata=writedata+1;

       except Exception as e:
              print("nsrllookup - Database NSRL table in NSRL db - Error: %s" % e)
              sys.exit(1)
       con.close() 
       if match == 0:
              dataline.append(linedata[0].decode("utf8"))
...
def reviewlookup(lineimport,writedata,reviewlist,dbtype):
   # Searches for matches to Review/Whitelist SHA1 hashes
   #lineimport=filter(lambda x: x in string.printable, lineimport)
    linedata = lineimport.split(None, 1)
    line = linedata[0]
    ...
    for dbitem in reviewlist:
       ...
       try: 
            with reviewcon:
                ...
                if 'filecheckcon' in globals():
                    sqlstr = "SELECT * from "+dbtype+" WHERE `FileName` = ? "  + combinesettings
                else:
                    sqlstr = "SELECT * from "+dbtype+" WHERE `SHA-1` = ? "  + combinesettings
...
# Malware Bazzar DB
#===========================
def malwarelookup(lineimport,writedata,malwarelist,dbtype):
   # Searches for matches to Review/Whitelist SHA1 hashes
   #lineimport=filter(lambda x: x in string.printable, lineimport)
    linedata = lineimport.split(None, 1)
    line = linedata[0]
    ...
    for dbitem in malwarelist:
       if os.path.isfile(dbitem):
          reviewcon = lite.connect(dbitem)
          #print("reviewcon set")
       else:
          print ("\n\nError: Malware SQLite database does not exist.")
          sys.exit(1)   
       ...
       try: 
            with reviewcon:
                ...
                if 'filecheckcon' in globals():
                    ...
                    #sqlstr = "SELECT * from "+dbtype+" WHERE `SHA-1` = ? "  + combinesettings
                    sqlstr = "SELECT * from "+dbtype+" WHERE `sha1_hash` = ? "  + combinesettings
                    #print(sqlstr)
                ...
                if row:
                    dataline.append(linedata[0].decode("utf8")) #sha1
                    dataline.append("") #sha256
                    dataline.append("") #MD5
                    dataline.append("") #CRC32
                    dataline.append(row["file_name"]) #Filename from DB
                    dataline.append("")  #File Size
                    dataline.append("VT %: " +  row["vtpercent"] + "  https://virustotal.com/gui/file/"+row["sha256_hash"]) #Application Type
                    dataline.append("")  #ProductName
                ...
                if "list index out of range" in e:
                   ...
                   print("Database review table in malware db - Error: %s" % e)
                   print("Error on lookup: %s" % srchline)
                   sys.exit(1)
       reviewcon.close()
    if writedata == 0:
       if 'filecheckcon' in globals():
...
def main(argv):
   ...
   print("IR Distill")   
   print("A NSRL/Review/Whitelist/Malware Comparison Reducer")
   ...
   parser.add_argument('-n', '--nsrl', nargs='*', dest='nsrl', help='NSRL SQLite database(s) to check against', required=False)
   ...
   parser.add_argument('-m', '--malwarelist', nargs='*', dest='malwarelist',help='SQLite database(s) of malware', required=False)
   ...
   print("Malware List SQLite filename: %s" % args.malwarelist)
   ...
   nsrllistdb = []
   if args.nsrl:
       for filedir in args.nsrl:
           if os.path.isfile(filedir):
               nsrllistdb.append(filedir)
           if os.path.isdir(filedir):
               ...
               for testfile in onlyfiles:
                   ...
                   if 'application/x-sqlite3' in kind.mime:
                       nsrllistdb.append(checkf)
                       print('\tFile %s is a sqlite db.' % checkf)
   ...
   malwarelistdb = []
   if args.malwarelist:
       for filedir in args.malwarelist:
           if os.path.isfile(filedir):
               malwarelistdb.append(filedir)
           if os.path.isdir(filedir):
               from os import listdir
               from os.path import isfile, join
               onlyfiles = [f for f in listdir(filedir) if isfile(join(filedir, f))]
               print('Searching for sqlite dbs: %s' % onlyfiles)
               for testfile in onlyfiles:
                   checkf = filedir + "/" + testfile
                   #print('Checkf: %s' % checkf)
                   #sys.exit(1)
                   kind = filetype.guess(checkf)
                   if kind is None:
                      #print('File %s is not a sqlite3 database' % checkf)
                      continue
                   #print('File MIME type: %s' % kind.mime)
                   if 'application/x-sqlite3' in kind.mime:
                       malwarelistdb.append(checkf)
                       print('\tFile %s is a sqlite db.' % checkf)
   ...
   if args.filecheck:
       match.write(u'Freq. Cnt|Filename|Product Version|DB Source\n')
       nomatch.write(u'Freq. Cnt|Filename\n')
   else:
       match.write(u'SHA1|SHA256|MD5|CRC32|FileName from DB|FileSize|ApplicationType|ProductName|Product Version|DB Source\n')
   ...
   with open(args.infile, "rU") as fileread:
       for line in fileread:
          ...
          if (args.nsrl) or (args.reviewlist) or (args.malwarelist) or (args.nwe):
             ...
             if len(line)==40:
                if args.nsrl:
                   ...
                   dataline,writedata = nsrllookup(line,writedata,nsrllistdb)  
                   
                   #dataline,writedata = reviewlookup(line,writedata,nsrllistdb,"nsrl")
                   #print("**dateline: %d**" % len(dataline))
                   #print("**writedate: %d**" % writedata)
                   #if args.showallmatches and not args.filecheck:
                   #    writeout(dataline,writedata) 
                   if writedata > 0:
                      writeout(dataline,writedata)
                if (args.showallmatches) or (writedata < 1):
                   ...
                   if args.reviewlist:
                      #print("review")
                      dataline,writedata = reviewlookup(line,writedata,reviewlistdb,"review")  
                      #if writedata > 0:
                         #writeout(dataline,writedata)
                   if args.malwarelist:
                      #print("malware")
                      dataline,writedata = malwarelookup(line,writedata,malwarelistdb,"malware")  
                      #if writedata > 0:
                         #writeout(dataline,writedata)
                if (args.showallmatches) or (writedata < 1):
                   #if writedata < 1:                         
