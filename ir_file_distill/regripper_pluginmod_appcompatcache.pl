...
#
# Mods for appcompatcache list by K Murphy
#
#-----------------------------------------------------------
...
sub pluginmain {
	...
	::rptMsg("Altered appcompatcache v.".$VERSION); # banner
...
# file
	...
	if ($key = $root_key->get_subkey($key_path)) {
                ...
                if ($cn = $root_key->get_subkey($cn_path)) {
                        $computername = $cn->get_value("ComputerName")->get_data();
...
#				::rptMsg("");
				$singleline = $f;
				$singleline .= "|ModTime: ".gmtime($files{$f}{modtime})." Z";
				if (exists $files{$f}{updtime}) {
				   $singleline .= "|UpdTime: ".gmtime($files{$f}{updtime})." Z";
				}
				if (exists ($files{$f}{size})) {
				   $singleline .= "|Size   : ".$files{$f}{size}." bytes";
				}
				if (exists ($files{$f}{executed})) {
				   $singleline .= "|Executed";
				}
				$singleline .= "|".$computername;
				::rptMsg($singleline);
				push(@Shimlist,$singleline);
				::rptMsg($f);
				push(@temps,$f) if (grep(/[Tt]emp/,$f));
			}
			
			if (scalar(@Shimlist) > 0) {
			        ::rptMsg("APPCACHE CSV list sorted by path");
			        my @Shimlistsort = sort { "\L$a" cmp "\L$b" } @Shimlist;
				foreach (@Shimlistsort) {
					::rptMsg("APPCACHE:".$_);
				}
			}			
