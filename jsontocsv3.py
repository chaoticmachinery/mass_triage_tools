...
while True:
    ...
    if count > 0:
       ...
       df.to_csv(outputfile, columns = header, encoding='utf-8', sep='|',doublequote=True,index=False,header=False,mode='a')
    else: 
       #df.to_csv(outputfile, columns = header, encoding='utf-8', sep='|',quotechar = "~",index=False,header=True,mode='w') 
       df.to_csv(outputfile, columns = header, encoding='utf-8', sep='|',doublequote=True,index=False,header=True,mode='w')
    df = pd.DataFrame()  
readfile.close()
