...
def reviewlookup(lineimport,writedata):
    ...
    try: 
        ...
        with reviewcon:
            ...
            if 'filecheckcon' in globals():
                ...
                #print("Writedata: %d" % writedata)
            else:
                sqlstr = "SELECT * from review WHERE `SHA-1` = ? "  + combinesettings
                #print("Writedata1: %d" % writedata)
            ...
            if row is None:
                ...
                dataline.append("")
                dataline.append("")  
                dataline.append('review/whitelist')   
                writedata=writedata+1;
...
def main(argv):
   ...
   parser.add_argument('-n', '--nsrl', help='SQLite database to check against', required=False)
   parser.add_argument('-f', '--filecheck', help='SQLite database to check against', required=False)
   ...
   parser.add_argument('-r', '--reviewlist', help='SQLite database of reviewed binaries/whitelist', required=False)
   ...
   if args.reviewlist:
     if os.path.isfile(args.reviewlist):
         global reviewcon
         reviewcon = lite.connect(args.reviewlist)
     else:
   ...
   if args.filecheck:
       match.write(u'Freq. Cnt|SHA1|MD5|CRC32|FileName|FileSize|ApplicationType|ProductName|DB Source\n')
       nomatch.write(u'Freq. Cnt|Filename\n')
   else:
       match.write(u'SHA1|MD5|CRC32|FileName|FileSize|ApplicationType|ProductName|DB Source\n')
   ...
   with open(args.infile, "rU") as fileread:
       for line in fileread:
          ...
          if args.filecheck:
             ...
             if writedata < 1:
                if args.reviewlist:
                   dataline,writedata = reviewlookup(line,writedata)
