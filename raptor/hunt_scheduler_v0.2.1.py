...
version = '0.2.1'
#Setting expire time to 6  months
expiretime = 15780000  
...
def main():
    ...
    with open(args.csv) as csvfile:
        ...
        for row in reader:
           ...
           args.query = 'SELECT hunt(description=\"' + row['artifact'] + '\", artifacts=\'' + row['artifact'] + '\',  timeout=' + row['time'] + ', pause='+row['pause']+', cpu_limit=' + row['cpu_limit'] + ', expires=now() + '+str(expiretime)+') FROM scope()'
